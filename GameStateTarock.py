...
class GameStateTarock(TarockBasics):
    ...
    def points_per_player(self, stack, players):
        win_i = self.winning_card(stack)
        winning_player = players[win_i]

        points = self.eval_stack(stack)
        ...
        if winning_player in self.duo:
            if first in self.duo:
                result[first] += points
            if second in self.duo:
                result[second] += points
            if third in self.duo:
                result[third] += points
        else:
            result[winning_player] += points
    ...
    def update_state(self, card, player):
        ...
        elif card in self.player3_cards:

...
class MilestoneAgents(GameStateTarock):
    ...
    def klop_legal(self, stack, hand):
        if len(stack) == 0:
            return hand
        else:
            legal = self.can_pickup_stack(stack, hand)
            if len(legal) > 0:
                return legal
            else:
                return self.legal_moves(stack[0], hand)

    def klop_agent(self, player_id):
        legalne = self.klop_legal(self.stack, self.player_hands[player_id])

        if 40 in legalne and len(legalne) > 1:
            tru_legalne = legalne[:legalne.index(40)] + legalne[legalne.index(40) + 1:]

        if (40 in legalne and len(legalne) == 1) or (40 not in legalne):
            tru_legalne = legalne

        return random.choice(tru_legalne)
...
if __name__ == "__main__":
    ...
    gst = MilestoneAgents(p1, p2, p3, [1, 2, 3], 1, [1, 3])
    ...
    children = []
    tree = Tree(-1, None, None, children, 3, gst)



    for c in p1:
        children2 = []
        nivo1 = Tree(c, True, tree, children2, 2, gst)
        children.append(nivo1)
        for c2 in p2:

            children3 = []
            nivo2 = Tree(c2, True, nivo1, children3, 1, gst)
            children2.append(nivo2)

            for c3 in p3:
                children3.append(Tree(c3, False, nivo2, None, 0, gst))
